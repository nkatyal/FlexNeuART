...
class TrainSamplerFixedChunkSizeUnique:

    """
        A helper class to sample training data in chunks of fixed size, which include
        one positive and multiple negative document IDs.
        This differs from TrainSamplerFixedChunkSize in the regard that training pairs 
        are not repeated (negatives are not repeated even across epochs as long as there 
        are sufficient negative documents)
    """
    def __init__(self,
                     train_pairs:dict,
                     neg_qty_per_query:int,
        ...
        self.qnum = -1
        self.positive_docs_seen = {} # {qid : set(pos_docs_id,...)}
        self.negative_docs_seen = {} # {qid : set(neg_docs_id,...)}
    ...
    def __iter__(self):
        self.qnum = -1
        self.step = 0
        self.positive_docs_seen = {}
        self.negative_docs_seen = {}
        return self
    ...
    def __next__(self) -> TrainSample:
        ...
        while self.step < self.step_qty:
            ...
            if len(pos_ids) < 1:
                continue
            # Select a positive document id that has not been seen before
            if self.positive_docs_seen.get(qid) is None or len(self.positive_docs_seen[qid]) == len(pos_ids):
                self.positive_docs_seen[qid] = set()

            pos_id = None

            while(pos_id is None or pos_id in self.positive_docs_seen[qid]):
                pos_id = np.random.choice(pos_ids, size=1)[0]

            self.positive_docs_seen[qid].add(pos_id)

            pos_ids_lookup = set(pos_ids)

            # Select negative document ids that have not been seen before
            if self.negative_docs_seen.get(qid) is None:
                self.negative_docs_seen[qid] = set()
            
            all_neg_id_arr = [did for did in query_train_pairs if did not in pos_ids_lookup and did not in self.negative_docs_seen[qid]]
            all_neg_qty = len(all_neg_id_arr)
            if all_neg_qty < 1:
                continue
            if all_neg_qty >= self.neg_qty_per_query:
...
class TrainSamplerFixedChunkSizeGroupByQuery:

    """
        A helper class to sample training data in chunks of fixed size, which include
        one positive and multiple negative document IDs. 
        Exhausts all the positive training pairs for a given query before moving on.
    """
    def __init__(self,
                     train_pairs:dict,
                     neg_qty_per_query:int,
    ...
    def __next__(self) -> TrainSample:
        ...
        while self.step < self.step_qty:
            ...
            if self.positive_docs_seen.get(qid) is None:
                self.positive_docs_seen[qid] = set()
            
            pos_ids = [did for did in query_train_pairs if self.qrels.get(qid, {}).get(did, 0) > 0]
            # Either insufficient positive training pairs or all the positive training pairs have been used
            if len(pos_ids) < 1 or len(self.positive_docs_seen[qid]) == len(pos_ids):
                self.positive_docs_seen[qid] = set()
                self.qnum += 1
                continue
